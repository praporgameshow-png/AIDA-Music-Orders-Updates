...
function Is-SupportedUrl([string]$Url) {
    ...
    $low = $Url.Trim().ToLowerInvariant()
    return ($low.Contains("youtu.be/") -or $low.Contains("youtube.com/") -or $low.Contains("music.youtube.com/") -or $low.Contains("vkvideo.ru/") -or $low.Contains("vk.com/video") -or $low.Contains("vk.ru/video") -or $low.Contains("music.yandex.ru/") -or $low.Contains("music.yandex.com/"))
}

function Get-SupportedUrl($Data, [string]$RawText) {
    ...
    if (-not [string]::IsNullOrWhiteSpace($RawText)) {
        $matches = [regex]::Matches($RawText, 'https?://[^\s"''<>\\]+', [Text.RegularExpressions.RegexOptions]::IgnoreCase)
...
function Load-Seen([int]$TtlSeconds) {
    ...
    foreach ($k in @($result.Keys)) {
        if (($now - [long]$result[$k]) -gt $TtlSeconds) { $result.Remove($k) }
    }
    return $result
}
...
function Submit-ToAida([string]$ApiUrl, [string]$UserName, [string]$Url) {
    ...
    try {
        ...
        return [pscustomobject]@{ Accepted = $false; Message = "AIDA Music Orders не ответила"; ErrorCode = "api_unavailable" }
    }
}
...
Write-Host "========================================================"
Write-Host " AIDA Music Orders v1.0.4 - MiniChat Bridge RC2"
Write-Host "========================================================"
...
while ($true) {
    ...
    try {
        ...
        while ($ws.State -eq [System.Net.WebSockets.WebSocketState]::Open) {
            ...
            $data = $frame.Data
            if ($null -eq $data -or ([string]$data.Type) -ine "Reward") { continue }
            if ((Normalize-Text ([string]$data.Name)) -ne $rewardNormalized) { continue }

            $service = [string]$data.Service
            if ($config.service_aliases -and @($config.service_aliases).Count -gt 0) {
                $allowed = $false
                foreach ($alias in @($config.service_aliases)) { if ($service -ieq [string]$alias) { $allowed = $true; break } }
                if (-not $allowed) { Write-BridgeLog ("Награда AIDA MUSIC пришла из неподдерживаемого сервиса: " + $service) "WARN"; continue }
            }

            $userName = [string]$data.UserName
            if ([string]::IsNullOrWhiteSpace($userName)) { $userName = "Неизвестный зритель" }
            $url = Get-SupportedUrl $data $raw
        ...
        Write-BridgeLog ("Соединение потеряно: " + $_.Exception.Message) "WARN"
    } finally {
        if ($null -ne $ws) { try { $ws.Abort() } catch {}; try { $ws.Dispose() } catch {} }
    }
    $wait = $backoff[[Math]::Min($attempt, $backoff.Count - 1)]
    Write-BridgeLog ("Повторное подключение через " + $wait + " сек...")
